import anyconfig
import copy
import os
import tempfile
from utils_ak.deployment.controller import Controller
from utils_ak.os import makedirs, execute
from utils_ak.split_file import SplitFile


class DockerController(Controller):
    def start(self, deployment):
        # create docker-compose file and run it without building

        dcc = {'version': '3', 'services': {}}
        for i, (name, container) in enumerate(deployment.config['containers'].items()):
            service = {'image': container['image'], 'tty': True, 'container_name': f'{deployment.deployment_id}_{i}'}

            service['command'] = []

            for k, v in container['command_line_arguments'].items():
                service['command'].append(f'--{k}')
                service['command'].append(v)
            dcc['services'][name] = service

        makedirs(f'data/{deployment.deployment_id}/')
        fn = f'data/{deployment.deployment_id}/docker-compose.yml'

        with open(fn, 'w') as f:
            f.write(anyconfig.dumps(dcc, 'yaml'))

        execute(f'docker-compose -f "{fn}" up -d --no-build')

    def stop(self, deployment):
        pass