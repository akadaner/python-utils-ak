from utils_ak.data.df.gran_df.sync import GranularSync
...
class GranularStreamer(object):
    ...
    def sync_with_base(self):
        ...
        # initialize new streamer with write mode - to remove current stream data
...
def ex_stream():
    ...
    print('-' * 100)
    import time
    from utils_ak.data.df.gran_df import GranularMsgPackStorage as Storage
    from utils_ak.os_tools import remove
    remove('data/gs_root/')
    gs = Storage('data/gs_root')
...
def ex_migration():
    ...
    remove('data/gs_from/')
    remove('data/gs_to/')
    gs_from = Storage('data/gs_from')
    gs_to = Storage('data/gs_to')
