import time
from utils_ak.messenger.slack.slack_messenger import (
    SlackMessenger,
    SimpleMessage,
    SlackMessage,
    Attachment,
...
def send_and_update_example():
    messenger.upd_message(
        "Message update test before the update", "#general", "message_unique_key"
    )
    time.sleep(5)
    messenger.upd_message(
        "Message update test after the update", "#general", "message_unique_key"
    )
